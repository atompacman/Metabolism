...
namespace Metabolism.World.Gravitational
{
   public class Tube : MonoBehaviour
   {
      ...
      private void Update()
      {
         if (m_NumSides != m_NumSidesPrevFrame ||
             !Mathf.Approximately(m_Skewness, m_SkewnessPrevFrame))
         {
            RegenerateQuads();
      ...
      private void RegenerateQuads()
      {
         ...
         for (int i = 0; i < m_NumSides; ++i)
         {
            ...
            quad.transform.forward = new Vector3(m_Skewness * Mathf.Cos(angleRad), 0,
               Mathf.Sin(angleRad));
            quad.transform.localScale =
               new Vector3(
                  m_Skewness * angleRadDelta *
                  Mathf.Sqrt(Mathf.Max(m_Skewness * Mathf.Sin(angleRadDelta),
                     Mathf.Abs(Mathf.Cos(angleRad)))), 1, 1);
         }
